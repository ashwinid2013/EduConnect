...
function ViewChildComponent() {
  ...
  const [classDetails, setclassData] = useState(null); // State to hold the response from the additional API call
    ...
    if (selectedStudentId) {
      fetch(`http://localhost:8080/getStudentById/${selectedStudentId}`)
        ...
        .then((data) => {
          ...
          return fetch(`http://localhost:8080/anotherEndpoint/${selectedStudentId}`);
        })
        .then((response) => response.json())
        .then((data) => {
            ...
            <div className="card-body p-4">
                  ...
                  <table className="table table-bordered mt-3">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          <strong>Standard:</strong> {classDetails.std_id}
                        </td>
                      </tr>
                    </tbody>
                  </table>
              ...
              {classDetails&& (
                <div className="mt-4">
                  ...
                  <p>{JSON.stringify(classDetails)}</p> {/* Display the additional data */}
                </div>
              )}
            </div>
          </div>
